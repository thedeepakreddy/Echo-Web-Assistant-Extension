...
// While an isolated task runs, browser tools are confined to this window.
let scope: { windowId: number } | null = null;

export function agentScope(): { windowId: number } | null { return scope; }
export function setAgentScope(next: { windowId: number } | null): void { scope = next; }
...
/** During an isolated task, is this tab inside the private window? */
export async function tabInScope(tabId?: number): Promise<boolean> {
  if (!scope) return true;
  if (tabId == null) return false;
  try { return (await chrome.tabs.get(tabId)).windowId === scope.windowId; } catch { return false; }
}

export async function assertInScope(tabId?: number): Promise<void> {
  if (!await tabInScope(tabId)) throw new Error('Isolated browsing can only use tabs in the private ECHO window.');
}

/** Private browsing only visits HTTPS sites. */
export function assertIsolatedUrl(url: string): void {
  if (scope && !/^https:\/\//i.test(url)) throw new Error('Isolated browsing only opens HTTPS pages.');
}
